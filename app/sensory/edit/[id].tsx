...
export default function EditSensoryProfileScreen() {
  ...
  const loadProfile = async () => {
    ...
    try {
      const profiles = await sensoryProfilesApi.getByChildId(selectedChild.id);
      ...
      if (profile) {
        ...
        setScores({
          ...
          vestibular: profile.vestibular,
          proprioceptive: profile.proprioceptive,
          oral: profile.oral,
        });
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await sensoryProfilesApi.update(id as string, {
        date,
      ...
      if (typeof window !== 'undefined') {
        window.alert('감각 프로파일 수정 중 오류가 발생했습니다.');
      } else {
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>감각 프로파일 수정</Text>
        <View style={{ width: 50 }} />
      </View>
