...
const CATEGORIES = ['전체', '수당/지원금', '보육', '교육', '돌봄', '출산/임신', '건강/의료', '다자녀/가족'];

export default function ChildAlertsScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [posts, stats, bmIds] = await Promise.all([
        fetchWelfareAlerts(100, 0, 'child'),
        fetchWelfareStats('child'),
        getBookmarkIds(),
      ]);
      setAlerts(posts);
  ...
  const handleSearch = async (text: string) => {
    ...
    if (text.trim().length >= 2) {
      try {
        const results = await searchWelfareAlerts(text.trim(), 50, 'child');
  ...
  if (loading) {
    return (
      <View style={[styles.center, ds.container]}>
        ...
        <Text style={[styles.loadingText, ds.meta]}>아동 복지 정보를 불러오는 중...</Text>
      </View>
    );
  }
    ...
    <View style={[styles.container, ds.container]}>
      <View style={[styles.header, ds.header]}>
        <View style={styles.headerRow}>
          ...
          <Text style={[styles.headerTitle, ds.headerTitle]}>아동 복지 알림</Text>
          <View style={{ width: 60 }} />
        </View>
