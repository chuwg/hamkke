...
- 오늘 일정 / 다가오는 일정 표시
- 자녀 나이 표시 (N세 M개월)
- 활동 요약: 마지막 치료, 이번 달 치료 시간, 마일스톤 달성률
- 통계 카드
- 빠른 액션 버튼 (일정, 치료기록, 마일스톤, 감각평가 등)

### 정보 탭
- 장애아동 복지 알림 (welfare-bot 연동)
- 아동 복지 알림
- 즐겨찾기 (북마크) 기능
...
│   ├── child/             # 자녀 관리 화면
│   ├── info/              # 정보 화면 (학교, 복지시설, 지원서비스, 복지알림, 즐겨찾기)
│   ├── milestone/         # 마일스톤 화면
│   ├── schedule/          # 일정 화면
│   ├── sensory/           # 감각 프로파일 화면
│   ├── therapy/           # 치료 기록 화면
...
├── services/              # 서비스
│   ├── localStorage.ts    # AsyncStorage CRUD + CSV 내보내기
│   ├── bookmarkApi.ts     # 즐겨찾기 관리
│   ├── welfareAlertApi.ts # 복지 알림 API
│   ├── calendar.ts        # 네이티브 캘린더
│   ├── notifications.ts   # 푸시 알림
│   ├── welfareApi.ts      # 복지시설 API
│   └── schoolInfoApi.ts   # 학교정보 API
├── utils/                 # 유틸리티 함수
│   └── dateFormat.ts      # 날짜 포맷 (통합 유틸리티)
└── types/                 # TypeScript 타입
    └── index.ts           # 타입 정의
```
...
### 2026-03-16 (v1.1.0)
- **일정 완료 토글**
  - 일정 카드에 체크박스 추가 (완료 시 초록 체크마크)
  - 완료된 일정: 제목 취소선 + 카드 흐리게 표시

- **홈 대시보드 강화**
  - 자녀 나이 표시 (N세 M개월)
  - 활동 요약 카드: 마지막 치료 날짜/유형, 이번 달 총 치료 시간, 마일스톤 달성률
  - 빈 일정 상태 개선 ("오늘은 여유로운 하루예요" + 일정 추가 바로가기)

- **검색 기능 추가**
  - 일정 검색 (제목, 설명)
  - 치료 기록 검색 (치료 유형, 치료사, 메모)
  - 마일스톤 검색 (마일스톤명, 메모)

- **복지 알림 연동**
  - 장애아동 복지 알림 (welfare-bot API 연동)
  - 아동 복지 알림
  - 즐겨찾기 (북마크) 기능 - AsyncStorage 기반 영구 저장
...
- **코드 품질 개선**
  - 날짜 포맷 함수 통합 (utils/dateFormat.ts로 5개 파일 중복 제거)
  - console.log/error 전체 제거 (프로덕션 정리)
